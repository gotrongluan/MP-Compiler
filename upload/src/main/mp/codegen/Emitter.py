...
class Emitter():
    ...
    def emitPUSHFCONST(self, in_, frame):
        ...
        #frame: Frame
        if type(in_) is str:
            return self.emitPUSHFCONST(float(in_), frame)
        frame.push()
        rst = "{0:.4f}".format(in_)
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        frame.pop()
        objectType = [cgen.ArrayPointerType, cgen.ClassType, ArrayType, StringType]
    ...
    def emitCLONE(self, in_, frame):
        result = list()
        result.append(self.emitINVOKEVIRTUAL(self.getJVMType(in_) + '/clone', MType(list(), cgen.ClassType('java/lang/Object')), frame))
    ...
    def emitNEWARRAY(self, in_, frame):
        ...
        #frame: frame
        result = list()
        numEle = in_.upper - in_.lower + 1
        result.append(self.emitPUSHICONST(numEle, frame))
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) in objectType:
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        frame.pop()
        objectType = [cgen.ClassType, ArrayType, StringType]
        if type(inType) in [IntType, BoolType]:
    ...
    *   @param in the type descriptor of the method.
    '''
    def emitINVOKESPECIAL(self, frame):
        #lexeme: String
        #in_: Type
        #frame: Frame
        frame.pop()
    ...
    *   @param in the type of the operands.
    ...
    def emitANDOP(self, frame):
        ...
        result = list()
        frame.pop()
        result.append(self.jvm.emitIMUL())
        result.append(self.emitIFFALSE(label1, frame))
    ...
    *   generate ior
    '''
    def emitIOROP(self, frame):
        #frame: Frame

        frame.pop()
        return self.jvm.emitIOR()

    def emitIREOP(self, op, frame):
        result = list()
    ...
    def emitFREOP(self, op, frame):
        ...
        if op == '>':
            result.append(self.jvm.emitIFLE(label1))
        elif op == '>=':
            result.append(self.jvm.emitIFLT(label1))
        elif op == '<':
            result.append(self.jvm.emitIFGE(label1))
        elif op == '<=':
            result.append(self.jvm.emitIFGT(label1))
        elif op == '=':
            result.append(self.jvm.emitIFNE(label1))
        elif op == '<>':
            result.append(self.jvm.emitIFEQ(label1))
        result.append(self.emitPUSHICONST(1, frame))
        frame.pop()
        result.append(self.emitGOTO(label2, frame))
        result.append(self.emitLABEL(label1, frame))
    ...
    def emitREOP(self, op, in_, frame):
        ...
        #..., value1, value2 -> ..., result
        if type(in_) is IntType:
            return self.emitIREOP(op, frame)
        return self.emitFREOP(op, frame)
